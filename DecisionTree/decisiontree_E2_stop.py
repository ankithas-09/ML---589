...
class DecisionTree:
    ...
    def construct_tree(self, data, attr_list):
        node = Node()
        if len(attr_list) == 0 or data['target'].nunique() <= 1 or self.should_stop(data):
    ...
    def find_split(self, data, attributes):
        ...
        for attr in attributes:
            ...
            entropies = [count * self.entropy(data[data[attr] == value]) for value, count in zip(unique_attr_values, count_of_attr_values)]
            entropy_for_attr = sum(entropies) / sum(count_of_attr_values)
            info_gains[attr] = total_entropy - entropy_for_attr

        best_attr = max(info_gains, key=info_gains.get, default=attributes[0])
        return best_attr

    # Function to determine whether to stop splitting
    def should_stop(self, data):
        majority_class_ratio = data['target'].value_counts(normalize=True).max()
        return majority_class_ratio >= 0.85
